---
title: Statistiques au niveau d'un département
---

```departements_values
SELECT 
    distinct departement_label
from escosol.parcs
...
<Dropdown 
    data={departements_values} 
    name=departement_selector 
    value=departement_label 
    title="Département" 
/>
...
```parcs_filtered_for_map
  select
      *,
      coalesce("Usage initial",'Usage inconnu') as usage_clean
  from escosol.parcs
  where "Longitude" is not null and "Latitude" is not null
  and departement_label='${inputs.departement_selector.value}'
...
En **{inputs.departement_selector.value}** :

```stats_global_departement
SELECT 
...
FROM escosol.parcs
where departement_label='${inputs.departement_selector.value}'
```

<BigValue 
  data={stats_global_departement} 
  value=nombre_parcs
...
<BigValue 
  ...
  fmt='#,##0 "ha"'
  comparison=ratio_missing_surface
  comparisionFmt='#,##0 "%"'
  comparisonTitle='% de valeurs manquantes'
...
<BigValue 
  ...
  fmt='#,##0 "MWc"'
  comparison=ratio_missing_puissance
  comparisionFmt='#,##0 "%"'
  comparisonTitle='% de valeurs manquantes'
  comparisonDelta=false
  minWidth='30%'
...
<BigValue 
  ...
  fmt='#,##0 "km"'
  comparison=ratio_missing_raccordement
  comparisionFmt='#,##0 "%"'
  comparisonTitle='% de valeurs manquantes'
  comparisonDelta=false
/>
...
<ECharts config={
    {
      color: ['#C65D47', '#5BAF7A', '#4A8EBA', '#D35B85', '#E1C16D', '#6F5B9A', '#147538', '#115759'],
      title: {
        text: `Surface occupée en fonction de l'usage initial`,
        left: 'center'
      },
      legend:{show:true},
      tooltip: {
